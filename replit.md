...
- **✅ AlifPay Payment Integration**: Implemented secure AlifPay payment gateway integration following the same security pattern as Payler. Created dedicated alifController.ts with proper HMAC signature validation, callback handling, and order status management. Added environment variables ALIF_MERCHANT_KEY, ALIF_MERCHANT_PASSWORD, and ALIF_API_URL. Integration uses web form submission to https://web.alif.tj/ with secure token generation and callback verification.
