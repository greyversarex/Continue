...
- **✅ AlifPay WebCheckout Integration**: Implemented full Alif WebCheckout API integration with proper HMAC-SHA256 password hashing, direct API calls to `/payments` endpoint, and structured callback handling. System now sends hashedPassword (HMAC-SHA256 of merchant password) in API requests, processes "PAID"/"FAILED" status responses, handles payment redirects to https://web.alif.tj/checkout/<token>, and maintains order tracking with email confirmations. All amounts converted to tiyin (x100) as required by Alif Bank specifications.
