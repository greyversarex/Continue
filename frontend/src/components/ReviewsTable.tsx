...
const ReviewsTable: React.FC = () => {
  ...
  const fetchReviews = async () => {
    try {
      setLoading(true);
      const response = await axios.get<ApiResponse>('http://localhost:5000/api/tours/reviews');
  ...
  const handleModerate = async (reviewId: number, isModerated: boolean) => {
    try {
      ...
      const response = await axios.put<ApiResponse>(`http://localhost:5000/api/tours/reviews/${reviewId}`, {
        isModerated
      });
